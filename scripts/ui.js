...
export function mostrarGestorRecetas() {
    ...
    document.getElementById('nueva-receta-btn').onclick = function() {
        contenido.innerHTML = `<h2 style='color:#4CAF50;text-align:center;'>Nueva Receta</h2>` +
            `<form id='form-nueva-receta'>` +
            `<div><label>Equipo: <input type='text' name='equipo' required></label></div>` +
            `<div><label>Clase: <input type='text' name='clase' required></label></div>` +
            `<div><label>Nivel: <input type='text' name='nivel' required></label></div>` +
            `<div><label>Color: <input type='text' name='color' required></label></div>` +
            `<div><label>Base: <input type='text' name='base' required></label></div>` +
            `<div><label>Material 1: <input type='number' name='mat1' min='1' value='1' required></label></div>` +
            `<div><label>Material 2: <input type='number' name='mat2' min='1' value='1' required></label></div>` +
            `<div><label>Material 3: <input type='number' name='mat3' min='1' value='1' required></label></div>` +
            `<div><label>Material 4: <input type='number' name='mat4' min='1' value='1' required></label></div>` +
            `<div><label>Tasa de éxito (%): <input type='number' name='tasaExito' min='1' max='100' value='100' required></label></div>` +
            `<div style='margin-top:10px;'><button type='submit' style='background:#4CAF50;color:#fff;font-weight:bold;'>Guardar receta</button></div>` +
            `</form>` +
